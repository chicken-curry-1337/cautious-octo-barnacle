...
import type { IUserAbilities, TUser, UserAbility } from "../../entities/User/User.store";
...
export const CreateUser = observer(({createUser, defaultUser}: {createUser(data: TUser): void; defaultUser: TUser}) => {
    const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={styles.createUser}>
      <div className={styles.createUserField}>
        <span>Name:</span>
        <input {...register("playerName.name", {minLength: 1, maxLength: 5, required: true})} />
      </div>
      <div className={styles.createUserField}>
        <span>Surname:</span>
      <input {...register("playerName.surname", {minLength: 1, required: true})} />
      </div>
      {Object.keys(defaultUser.userAbilities).map((key) => 
      (<div className={styles.createUserField} key={key}>
        <span>{key}:</span>
        <input type="number" {...register(`userAbilities.${key as UserAbility}`, { min: 1, max: 15, required: true})} />
      </div>))}
